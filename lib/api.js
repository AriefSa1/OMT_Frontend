...
/**
 * Enqueue a sync job to run in the background instead of blocking the request.
 * Returns jobId immediately; poll /api/sync/jobs/:id for status.
 */
export async function triggerFullSyncAsync(storeId = null) {
  const result = await request('/sync/run-async', {
    method: 'POST',
    body: JSON.stringify({ store_id: storeId }),
  });
  if (result.success) clearApiCache();
...
export async function fetchSyncJobs(status = null) {
  const query = status ? `?status=${encodeURIComponent(status)}` : '';
  return request(`/sync/jobs${query}`);
}

export async function fetchSyncJob(jobId) {
  return request(`/sync/jobs/${encodeURIComponent(jobId)}`);
}
