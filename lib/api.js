...
async function request(path, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const timeoutMs = requestedTimeoutMs > 0
      ? requestedTimeoutMs
      : (attempt === 1 ? 65000 : 20000);
    const headers = { ...getAuthHeaders(), ...(options.headers || {}) };
    // Browser harus mengisi sendiri boundary multipart; kalau Content-Type dipaksa JSON,
    // middleware upload di backend tidak akan pernah melihat req.file.
    ...
    try {
      ...
      if (!response.ok) {
        // Pertahankan diagnostic backend agar UI dapat menampilkan alasan validator,
        // requestId, dan warning tanpa harus membuka terminal server.
        return {
          ...data,
          success: false,
          status: response.status,
          error: data.error || data.message || 'Permintaan gagal.',
        };
      }
      return data;
    } catch (error) {
...
// Analisa mendalam AI (Pusat Optimasi / Aksi & Tugas / Wawasan Growth berbagi 1 panggilan).
// Panggilan LLM lambat → timeout panjang & TANPA retry (mengulang buang kuota), lalu di-cache
// 10 menit supaya pindah antar-tab tidak memanggil model lagi.
export async function fetchActionAnalysis({ force = false } = {}) {
  const path = '/ai/action-analysis';
  const fetcher = async () => {
    try {
      const res = await fetchWithTimeout(`${API_BASE}${path}`, { cache: 'no-store', headers: getAuthHeaders() }, 120000);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) return { success: false, status: res.status, error: data.error || data.message || 'Gagal memuat analisa.' };
      return data;
    } catch (error) {
      return { success: false, error: error.name === 'AbortError' ? 'Analisa AI lama merespons — coba lagi sebentar.' : 'Server tidak dapat dihubungi.' };
    }
  };
  if (force) return fetcher();
  return readCached(path, fetcher, 10 * 60 * 1000);
}
...
export async function handleFileUpload(file) {
  ...
  return request('/admin/notifications/upload', { method: 'POST', body: formData });
}

export async function fetchDashboardOverview(storeId = null, period = 'real_time', { startDate, endDate } = {}) {
  const query = new URLSearchParams({ period });
  if (storeId) query.set('store_id', storeId);
  // Bila rentang tanggal dikirim, backend memfilter KPI/salesTrend/orderQuality ke rentang itu.
  if (startDate && endDate) { query.set('start_date', startDate); query.set('end_date', endDate); }
...
export async function updateShopeeCookie(rawCookie, storeName = '', marketplace = null) {
  const body = { rawCookie, storeName };
  if (marketplace && marketplace.id) {
    body.marketplaceId = marketplace.id;
    body.marketplaceName = marketplace.name || '';
    body.marketplaceType = marketplace.type || '';
  }
  const result = await request('/shopee/cookie', { method: 'POST', body: JSON.stringify(body) });
  if (result.success) clearApiCache();
  return result;
}

export async function fetchMarketplaces() { return cached('/warehouse/marketplaces', 300000); }

export async function updateStoreMarketplace(storeId, marketplace) {
  const body = marketplace && marketplace.id
    ? { marketplaceId: marketplace.id, marketplaceName: marketplace.name || '', marketplaceType: marketplace.type || '' }
    : { marketplaceId: null };
  const result = await request(`/shopee/stores/${storeId}/marketplace`, { method: 'PUT', body: JSON.stringify(body) });
  if (result.success) clearApiCache();
  return result;
}
...
// Menerima number (perilaku lama) ATAU objek { days, startDate, endDate }.
export async function fetchTrafficSources(daysOrOpts = 7, storeId = null) {
  const opts = typeof daysOrOpts === 'object' && daysOrOpts !== null ? daysOrOpts : { days: daysOrOpts, storeId };
  const query = new URLSearchParams();
  if (opts.startDate && opts.endDate) {
    query.set('start_date', opts.startDate);
    query.set('end_date', opts.endDate);
  } else {
    query.set('days', opts.days || 7);
  }
  const sid = opts.storeId || storeId;
  if (sid) query.set('store_id', sid);
  return cached(`/shopee/traffic-sources?${query.toString()}`, 60000);
}

// --- Endpoint baru: ringkasan funnel produk + tren metrik ---
export async function fetchProductOverview({ storeId = null, startDate, endDate, period } = {}) {
  const query = new URLSearchParams();
  if (startDate && endDate) { query.set('start_date', startDate); query.set('end_date', endDate); }
  if (period) query.set('period', period);
  if (storeId) query.set('store_id', storeId);
  return cached(`/shopee/product-overview${query.toString() ? `?${query}` : ''}`, 30000);
}
export async function fetchProductTrends({ storeId = null, startDate, endDate, period } = {}) {
  const query = new URLSearchParams();
  if (startDate && endDate) { query.set('start_date', startDate); query.set('end_date', endDate); }
  if (period) query.set('period', period);
  if (storeId) query.set('store_id', storeId);
  return cached(`/shopee/product-trends${query.toString() ? `?${query}` : ''}`, 30000);
}

// --- Promosi: diskon + voucher ---
export async function fetchDiscountPerformance({ storeId = null, startDate, endDate, promotionType, orderType } = {}) {
  const query = new URLSearchParams();
  if (startDate && endDate) { query.set('start_date', startDate); query.set('end_date', endDate); }
  if (promotionType !== undefined && promotionType !== null) query.set('promotion_type', promotionType);
  if (orderType) query.set('order_type', orderType);
  if (storeId) query.set('store_id', storeId);
  return cached(`/shopee/promotions/discounts${query.toString() ? `?${query}` : ''}`, 30000);
}
export async function fetchVoucherList({ storeId = null, offset = 0, limit = 10, promotionType = 2 } = {}) {
  const query = new URLSearchParams({ offset, limit, promotion_type: promotionType });
  if (storeId) query.set('store_id', storeId);
  return cached(`/shopee/promotions/vouchers?${query.toString()}`, 30000);
}
export async function fetchConnectionStatus() { return cached('/status', 20000); }
// Probe koneksi realtime (Toko/Gudang/AI) — tanpa cache, karena pemanggil melakukan polling sendiri.
export async function fetchConnectionHealth() { return request('/status/connections'); }
...
export async function fetchMarketplacePerformance({ startDate, endDate, storeId }) {
  const query = new URLSearchParams();
  if (startDate) query.append('startDate', startDate);
  if (endDate) query.append('endDate', endDate);
  if (storeId) query.append('store_id', storeId);
  const qStr = query.toString() ? `?${query.toString()}` : '';
  return cached(`/warehouse/marketplace-performance${qStr}`, 60000);
}
...
// Hermes Agent experiment — deliberately separate from /api/ai/* so the local agent
// cannot change the behavior or quota path of the production AI panels.
export async function fetchHermesStatus() {
  return request('/hermes/status');
}

export async function fetchHermesModels({ refresh = false } = {}) {
  return request(`/hermes/models${refresh ? '?refresh=true' : ''}`);
}

export async function sendHermesChat(messages, mode = 'EXPLORATORY', model = null) {
...
export async function validateHermesAnalysis(payload = {}) {
  return request('/hermes/analyze/validate', {
    method: 'POST',
...
export async function analyzeHermes(payload = {}) {
  return request('/hermes/analyze', {
    method: 'POST',
    body: JSON.stringify(payload),
    timeoutMs: 125000,
  });
...
export async function fetchHermesMemories(params = {}) {
  const query = new URLSearchParams();
  Object.entries(params).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== '') query.set(key, value);
  });
  return request(`/hermes/memories${query.toString() ? `?${query.toString()}` : ''}`);
}

export async function fetchHermesMemory(id) {
  return request(`/hermes/memories/${encodeURIComponent(id)}`);
}

export async function submitHermesFeedback(analysisId, payload = {}) {
  return request(`/hermes/analyze/${encodeURIComponent(analysisId)}/feedback`, {
    method: 'POST',
    body: JSON.stringify(payload),
    timeoutMs: 30000,
  });
...
export async function trackHermesAction(analysisId, recommendationIndex) {
  return request(`/hermes/analyze/${encodeURIComponent(analysisId)}/actions`, {
    method: 'POST',
    body: JSON.stringify({ recommendationIndex }),
  });
}

export async function updateHermesAction(actionId, payload = {}) {
  return request(`/hermes/actions/${encodeURIComponent(actionId)}`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  });
}

export async function deleteHermesAction(actionId) {
  return request(`/hermes/actions/${encodeURIComponent(actionId)}`, {
    method: 'DELETE',
    timeoutMs: 30000,
  });
...
export async function evaluateHermesAction(actionId, windowDays) {
  return request(`/hermes/actions/${encodeURIComponent(actionId)}/evaluate`, {
    method: 'POST',
...
// Async sync job queue API functions
export async function triggerFullSyncAsync(action = 'SYNC_ALL', storeId = null) {
  const body = { type: action, origin: 'MANUAL' };
  if (storeId) body.storeId = storeId;
  return request('/sync/run-async', { method: 'POST', body: JSON.stringify(body) });
}

// Enqueue sync ke job queue lalu POLL status (request pendek berulang) sampai selesai.
// Menghindari satu request HTTP panjang yang menggantung & di-abort saat sync lama.
export async function triggerSyncAndPoll({ action = 'SYNC_ALL', storeId = null, intervalMs = 2500, timeoutMs = 180000, onStatus } = {}) {
  const enq = await triggerFullSyncAsync(action, storeId);
  if (!enq?.success || !enq.jobId) {
    return { success: false, error: enq?.error || enq?.message || 'Gagal mengantre sinkronisasi.' };
  }
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    await new Promise((r) => setTimeout(r, intervalMs));
    const res = await fetchSyncJob(enq.jobId);
    const status = res?.job?.status;
    if (onStatus && status) { try { onStatus(status); } catch { /* abaikan */ } }
    if (status === 'COMPLETED') return { success: true, job: res.job };
    if (status === 'FAILED') return { success: false, error: res.job?.error || 'Sinkronisasi gagal.', job: res.job };
  }
  // Masih berjalan setelah timeout poll — bukan gagal; biarkan lanjut di background.
  return { success: true, pending: true, message: 'Sinkronisasi masih berjalan di latar belakang.' };
}

export async function fetchSyncJobs({ status, page = 1, limit = 20 } = {}) {
  const query = new URLSearchParams({ page, limit });
  if (status) query.set('status', status);
  return request(`/sync/jobs?${query.toString()}`);
}

export async function fetchSyncJob(jobId) {
  const result = await request(`/sync/jobs/${jobId}`);
  // Enhance response with computed duration if not present
  if (result?.job && result.job.startedAt) {
    const started = new Date(result.job.startedAt).getTime();
    const ended = result.job.completedAt ? new Date(result.job.completedAt).getTime() : Date.now();
    result.job.durationMs = result.job.durationMs ?? Math.max(0, ended - started);
  }
  return result;
}
