...
export async function fetchMarketplacePerformance({ startDate, endDate, page = 1, limit = 100 } = {}) {
  const query = new URLSearchParams({ page, limit });
  if (startDate && endDate) { query.set('start_date', startDate); query.set('end_date', endDate); }
  return cached(`/warehouse/marketplace-performance?${query.toString()}`, 60000);
}
