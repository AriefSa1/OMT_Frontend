...
// Penjelasan istilah untuk ikon info di header kolom.
const COL_TIPS = {
  gmv: 'Omzet dari pesanan terkonfirmasi selama periode.',
  avgOrderValue: 'AOV (Average Order Value) = Omzet ÷ jumlah order. Rata-rata nilai per pesanan.',
  gmvTrendPct: 'Perubahan omzet dibanding periode dengan panjang sama sebelumnya.',
  adsSpend: 'Total biaya iklan Shopee pada periode.',
  adsRoas: 'ROAS (Return on Ad Spend) = penjualan dari iklan ÷ biaya iklan. Makin tinggi makin efisien.',
  productCount: 'Jumlah produk terkatalog untuk toko ini.',
};
...
export default function AdminStoreAnalytics() {
  ...
  return (
    <div className="space-y-4">
      ...
      <DataSourceNote
        source="Shopee Seller Center — ringkasan pesanan & iklan"
        cadence="disinkronkan otomatis tiap ±15 menit"
        note="Metrik dihitung untuk periode terpilih; tren dibandingkan dengan periode sepanjang itu sebelumnya."
      />
                    ...
                    <span className={`inline-flex items-center gap-1 ${col.numeric ? 'flex-row-reverse' : ''}`}>
                      ...
                      {COL_TIPS[col.key] && (
                        <span onClick={(e) => e.stopPropagation()}>
                          <InfoTooltip label={col.label}>{COL_TIPS[col.key]}</InfoTooltip>
                        </span>
                      )}
                    </span>
                  </th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                Array.from({ length: 4 }).map((_, r) => (
                  <tr key={r}>
                    <td className="px-3 py-3.5"><span className="skeleton block h-4 w-4 rounded" /></td>
                    {COLUMNS.map((col) => (
                      <td key={col.key} className="px-4 py-3.5"><span className={`skeleton block h-3.5 ${col.numeric ? 'ml-auto w-12' : 'w-28'}`} /></td>
                    ))}
                  </tr>
                ))
              ) : sortedStores.length === 0 ? (
                <tr><td colSpan={COLUMNS.length + 1} className="px-4 py-12 text-center text-sm text-slate-500">
                  Belum ada toko terdaftar oleh pengguna.
                  <span className="mt-1 block text-xs text-slate-400">Statistik muncul setelah pengguna menambahkan toko Shopee dan sinkronisasi berjalan.</span>
                </td></tr>
              ) : (
                sortedStores.map((s) => {
                  const m = s.metrics || {};
